...
public class ReportController {
    ...
    public Result exportBusinessReport(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            String filepath = "D:\\files\\report_template.xlsx";
    ...
    public Result getMemberReport() {
        ...
        LinkedList<String> monthLists = new LinkedList();
        for (int i = 1;i < 12;i++) {
        ...
        map.put("months",monthLists);
        List<Integer> numList;
        try {
            numList = reportService.getMemberReport(monthLists);
        }catch (Exception e) {
            e.printStackTrace();
    ...
    public Result getSetmealReport() {
        ...
        LinkedList<String> setmealNames = new LinkedList<>();
        LinkedList<Integer> setmealIds = new LinkedList<>();
        for (Setmeal setmeal : setmealList) {
            setmealNames.add(setmeal.getName());
            setmealIds.add(setmeal.getId());
        }
        List<Integer> setmealCount;
        try {
            setmealCount = orderService.getSetmealCount(setmealIds);
        }catch (Exception e) {
            e.printStackTrace();
            return new Result(false,MessageConstant.GET_SETMEAL_COUNT_REPORT_FAIL,null);
        }
        Map<String,Object> setmealCountMap;
        List<Map> setmealCountList = new LinkedList<>();
        for (int i = 0;i < setmealNames.size();i++) {
            setmealCountMap = new HashMap<>();
            setmealCountMap.put("value",setmealCount.get(i));
            setmealCountMap.put("name",setmealNames.get(i));
            setmealCountList.add(setmealCountMap);
        }
